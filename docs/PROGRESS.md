...
See REVIEW.md for authoritative reviewed run paths. Earlier development runs
remain intact but may predate corrected metrics/interfaces. New generation never
overwrites them. The packaging script copies selected evidence into a unique
local directory, verifies available run hashes and writes a bundle manifest.

Physical hardware identification, measurements, fabrication, assembly, ejection
checks, and flight validation have not occurred. Demonstration assumptions must
not be interpreted as measured hardware or an approved flight design.
