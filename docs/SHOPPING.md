...
## Before buying for more performance

Read the [next-platform comparison and dated price examples](NEXT_PLATFORM.md). A 24 mm motor mount can fit a BT-60
airframe; a larger motor does not automatically require a larger body tube. Neither is a drop-in change to this model.
Hold motor/delay and mount purchases until the next measured or provisional configuration has been simulated with its
actual commercial thrust curves, mass/CG, packing, recovery and guide assumptions.

For the existing platform, generic uncut tube and measuring/fit-test supplies can still serve bench work. Avoid building
an entire 18 mm airframe solely because we already have CAD if the intended payload and performance are changing. No
full revised bill of materials or delivered-price comparison exists yet.

Follow manufacturer instructions, the [NAR Model Rocket Safety Code](https://www.nar.org/ModelRocketSafetyCode), and the
launch site's requirements with an experienced club/range officer. This project does not establish site legality,
suitability, or flight clearance.
